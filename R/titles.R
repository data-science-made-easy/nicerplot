titles <- function(p) {
  ...
  # x-title
  graphics::text(get_adj(p$x_title_align)$x, (p$margin_south - p$x_title_v_shift) / p$height, labels = set_newline(p$x_title), adj = get_adj(p$x_title_align)$adj, font = hack_font(p, 3), cex = p$x_title_font_size, col = get_col_from_p(p, p$x_title_col), family = p$font)
  ...
  # y-title "left"
  graphics::text(get_adj(p$y_title_align)$x, 1 - (p$margin_north - p$y_title_v_shift) / p$height, labels = set_newline(p$y_title), font = hack_font(p, 3), cex = p$y_title_font_size, adj = get_adj(p$y_title_align)$adj, col = get_col_from_p(p, p$y_title_col), family = p$font)
