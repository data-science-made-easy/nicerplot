if (file.exists('geo-participatie-gemeenten.xlsx')) { # plot figures from Excel file:
  ...
  nicerplot::nplot(d, style = c('map'), title = c('Participatie gemeenten'), footnote = c('Jaar 2018'), cbs_map = c('gemeente'), cbs_map_year = c(2018), name = c('niet;;wel'), legend_order = c(2, 1), open = FALSE)
