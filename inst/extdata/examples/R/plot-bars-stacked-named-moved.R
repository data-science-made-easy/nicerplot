if (file.exists('bars-stacked-named-moved.xlsx')) { # plot figures from Excel file:
  ...
  nicerplot::nplot(d, type = c('bar=, bar--, bar--, bar=, bar--, bar--, bar='), title = c('Number of phones in the world'), bar_stack_name = c('The Americas'), bar_stack_index = c(1), style = c('default'), open = FALSE)
