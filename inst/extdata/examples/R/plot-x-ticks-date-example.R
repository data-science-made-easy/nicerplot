if (file.exists('x-ticks-date-example.xlsx')) { # plot figures from Excel file:
  ...
  nicerplot::nplot(d, title = c('Groeibijdragen bestedingen'), y_title = c('%-punt bbp-groei'), footnote = c('x_ticks_date = years, quarters'), x_ticks_date = c('years, quarters'), style = c('default'), open = FALSE)
